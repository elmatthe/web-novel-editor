"""Renegade Immortal editorial pipeline.

Mirrors `pipelines/lord_of_mysteries.py` stage-for-stage — deliberately that one and not
`shadow_slave.py`/`supreme_magus.py`, because Renegade Immortal ran on the universal
fallback pipeline until 2026-07-26 and this promotion must not change its output beyond
the one thing it exists for. The transform sequence is identical, so the only behavioural
delta is that `RI_SPECIAL_FIXES` is now non-empty (the universal fallback resolves its
substitution map from `profiles/lord_of_mysteries`, which is shared by every profile-less
novel and pinned empty by test).

The canonical-name floor stays empty (`RI_CANONICAL_NAMES`); the novel's 837 protected
terms come from `scripts/Universal/resources/novel-index/renegade-immortal.txt` via the
registry/lexicon, exactly as before.
...
def _apply_special_fixes(text: str, repl_log) -> str:
    ...
    Longest-first is not cosmetic here: "Ligo" is a prefix of "Ligou", and applying the
    shorter key first would turn "Ligou" into "Liguou". See the profile module.
    """
    for bad, good in sorted(RI_SPECIAL_FIXES.items(), key=lambda kv: -len(kv[0])):
        if bad in text:
