"""Supreme Magus editorial pipeline (Phase 5b).

Mirrors `pipelines/shadow_slave.py` stage-for-stage — the same three-block
masked-correction structure documented there — driven by the Supreme Magus
profile data instead: `SM_CANONICAL_NAMES` (via the registry/lexicon) and
`SM_SPECIAL_FIXES` (corpus-validated proper-noun extraction artifacts:
Silverwing/Thrud/Orpal/Ragnarök misreads etc.). The legacy SM editor's
profanity-uncensor passes are spec-excluded and intentionally absent.
...
def _apply_special_fixes(text: str, repl_log) -> str:
    ...
    for bad, good in sorted(SM_SPECIAL_FIXES.items(), key=lambda kv: -len(kv[0])):
        if bad in text:
...
def run_pipeline(
    ...
    *,
    ...
    """Run the full Supreme Magus editorial pipeline over `text` and return clean text."""
    ...
    # (Supreme_Magus-v2 is the corpus these error pages were actually found in.)
