# Instructions — Phase 10: Registry Reconciliation, Junk-Strip Hardening, Editorial QA, Per-Novel Profiles, Scraper Alignment, and Repo Reorganization
...
1. `AI-WORKSPACE.md` — global conventions (phasing, verify gate, handoff, branch rules).
   **This file was recently expanded** — note especially the new fourth permanent doc
   `DECISIONS.md` (an append-only ADR log), the "Vibe Coding Workspace Files" section
   (scaffolder + templates + `Map-Repo-Structure.bat`), and the CSPW-PC portable-tools /
   no-admin environment notes.
...
3. `md-instructions/CHANGELOG.md` — history, especially v0.4.0 (junk_strip Stage 1.5
   ...
   dispatch registry).
4. `md-instructions/handoff.md` — most recent working state, open caveats. (Filename is
   lowercase in this repo — match the actual on-disk casing; don't rename it.)
...
6. `md-instructions/build-spec.md` and `md-instructions/decisions-template.md` — the build
   spec (source of the editing rules and the deliberate `files/` layout choice flagged in
   Phase 8) and the ADR template that seeds `DECISIONS.md`.
7. `files/Novel-Edits-Details/UNIVERSAL.md` and `Shadow-Slave.md` — the human-readable
   edit-details layer.
8. `files/study-examples/` — the user's older novel-specific editors/scrapers and per-novel
   word indices; source material for Phase 5b (see §2 "Per-novel editorial work" above).
9. `scripts/rules/junk_strip.py`, `scripts/core/novel_registry.py`,
   `scripts/core/edit_details.py`, `scripts/pipelines/shadow_slave.py`.
10. `REPO-STRUCTURE.md` at the repo root (generated by `Map-Repo-Structure.bat`) — the
    layout snapshot. Treat it as a *possibly-slightly-stale* map: verify against the live
    tree with `git status`/directory listing before relying on any specific path, since the
    user may have added corpora after it was generated.
11. **The actual current repository state** — do not trust any version/phase number stated
    ...
    (Note: earlier planning notes said `.claude/`/`.codex/` had been deleted; the current
    `REPO-STRUCTURE.md` shows both present *with* skills — so verify their actual state
    rather than assuming, and do not "restore" what already exists.)
12. The sibling project **`web-novel-scraper`** (public:
    `https://github.com/elmatthe/web-novel-scraper`) — the companion app that produces the
    PDFs this editor cleans. Read its `README.md`, `scripts/Universal/app.py` (GUI), and
    `scripts/Universal/webnovel_scraper/pdf_builder.py` (PDF layout). **Record the exact
    commit SHA you examined** in your Phase 0 baseline notes, since `main` is mutable and
    this plan's specific claims about that repo are tied to a point-in-time read. Treat
    everything read from that repo as a **visual/structural/behavioral reference**, not code
    to paste in verbatim — see Phase 7 §3 for a concrete example (a verified
    stale-comment/behavior mismatch in its GUI). **For launcher work (Phase 9), study this
    repo's own root `Setup_and_Run-template.bat` / `Setup_and_Run-template.command` as the
    pattern to follow** (they are the AI-WORKSPACE scaffolder sources) — but note these
    templates are **study copies only, not deliverables**: Phase 9 builds the single correct
    `Setup_and_Run.bat` / `Setup_and_Run.command` per the updated `AI-WORKSPACE.md` and then
    **deletes the `-template` files**. The shipped repo has exactly one launcher per OS.


---

## 1. The dual-mode model — reuse if present, build only if genuinely missing

The dual-mode editing model below is what the finished tool must do. **Whether it already
exists depends entirely on the Phase 0.5 branch reconciliation.** The public GitHub `main`
README describes it as shipped in v0.9.0, but the local clone Claude Code first checked was
an older v0.8.0 line without it. So: **do not assume it's already built, and do not rebuild
it blindly** — Phase 0.5 determines which. If the v0.9.0 registry is recovered from a
remote branch, this section describes what you're *confirming*; if it's genuinely
unrecoverable, this section describes what Phase 0.5's fallback *builds*.
...
- **Novel-specific edit mode** — layered on top when a registered novel (Shadow Slave, plus
  Noble Queen and Supreme Magus after Phase 5b) is selected in the GUI dropdown:
  protected-term masking (character names, factions, locations from
  `files/novel-index/<novel>.txt`), forced recurring typo substitutions
  (`<NOVEL>_SPECIAL_FIXES`), documented per-novel in
  `files/Novel-Edits-Details/<Novel-Name>.md`.

**Your job for the dual-mode switch specifically:** reuse the existing v0.9.0 registry if
Phase 0.5 recovers it (then stress-test it against the real corpora); build-and-prove it
only if Phase 0.5 confirms it's truly absent everywhere. Either way, everything else in this
plan (junk-strip, PDF, GUI, reorg, launcher, per-novel profiles) proceeds on top of it.
...
**Locate the available test corpora before assuming paths.** The `REPO-STRUCTURE.md`
snapshot (generated by `Map-Repo-Structure.bat`) reflects the repo at map time and may lag
what's actually on disk now. Confirmed state (from Claude Code's Phase-1 recon + the user's
additions):

- `files/pdf-example-chapters/webscraped_shadow_slave/` — ~3000 real Shadow Slave chapters.
  **Confirmed unusually clean:** a 100%-coverage scan of all 3,000 found **zero** URLs, bare
  domains, `@@…@@` markers, black-square/decorative-Unicode watermark letters, or
  spaced-domain promos. This source (freewebnovel) stripped its `<subtxt>` watermark tags at
  scrape time. Still the largest real Shadow Slave sample, useful for specific-mode testing
  at scale.
- `test-files/shadow_slave/` — the 10 pinned fixtures. **Bug confirmed:** these were never
  actually committed (`test-files/` is gitignored; `git ls-files` shows none), so
  fixture-backed tests silently skip in a clean clone. **The user has approved committing
  them** — Phase 2 un-ignores and commits these 10 small PDFs (and they still move to
  `files/test-files/` in Phase 8).
- **`files/pdf-example-chapters/` now also contains user-added Noble Queen and Supreme Magus
  example chapters** (added this round specifically for junk-strip hardening and Phase 5b
  profile authoring). **In Phase 1, locate and inventory these** (enumerate every
  novel-batch folder under `files/pdf-example-chapters/`, report exact paths/counts), scan
  them at 100% coverage, and use them as the junk-strip evidence base and as the corpora for
  the Noble Queen / Supreme Magus profile work.

**Junk-strip evidence base (resolved).** Since the Shadow Slave corpus is clean, the
evidence base comes from: (a) the newly-added Noble Queen and Supreme Magus example chapters
(scan them first — they may be from dirtier sources), and (b) the one genuinely-new,
user-documented fingerprint class Claude Code already found in the user's **own Noble Queen
scraper**: **webnovel.com decorative-Unicode watermark letters / homoglyphs** (e.g. ʀ ᴄ 𝙣
style substitutions in `_REPLACEMENTS`) that NFC normalization does **not** fold and current
`junk_strip` cannot catch. Phase 2 hardens against whatever the fresh scans reveal **plus**
this homoglyph-watermark class, and builds the full committed + optional-local-corpus test
infrastructure. If a scan still yields no new patterns for a given novel, say so — don't
invent patterns to justify the phase.

**Copyright / handling note:** all such chapter PDFs are copyrighted, kept local, and
gitignored (the 10 Shadow Slave fixtures are the one approved exception — small, and needed
so tests don't silently skip). Nothing else in this plan force-adds corpora to git, commits
full extracted chapter text, or quotes more than a short redacted fragment in any test, log,
or doc.

**Per-novel editorial work from `files/study-examples/` (Phase 5b).** The repo ships
`files/study-examples/` with per-novel word-list indices and the user's older editing/
scraping scripts. Confirmed contents from recon: **Noble Queen** — 26 real terms (its
`_legacy/noble_queen_do_not_touch.txt` is byte-identical to the master) plus the
`scrape_noble_queen*.py` scrapers that document source quirks; **Supreme Magus** — a
594-term master index + 4 legacy lists (richest data; **note:** its editor's
profanity-uncensor map is spec-excluded — do **not** port that); **Renegade Immortal** and
**Reverend Insanity** — master indexes are comment-only headers with **zero real terms**
(nothing to port; they stay universal-fallback placeholders). Phase 5b authors real profiles
for **Noble Queen and Supreme Magus** (the user's chosen two). This is a **data/porting
exercise** on the validated multi-novel seam — no new engine modules.
...
## Phase 0.5 — Branch Reconciliation: locate the real v0.9.0 registry BEFORE building anything

**Why this phase exists (resolved decision + a critical correction).** The user has
confirmed: *build the novel registry + Shadow Slave registration + universal-only
fallback.* **But do not build it blind.** Claude Code's Phase-1 recon found the local
`release-main @ 7a44b73` at **v0.8.0** with no `novel_registry.py`, no dispatch, and
`run_batch` applying Shadow Slave names/fixes to every novel. **However, the public GitHub
`main` README (github.com/elmatthe/web-novel-editor) already describes v0.9.0 as shipped:**
"a novel → pipeline dispatch registry: Shadow Slave runs its full profile, and any other
novel falls back to universal-only editing." So the v0.9.0 registry work **exists on GitHub
`main`** — the local clone Claude Code branched from was simply behind/on a different line
of history. Building a *fresh* registry now would create a conflicting parallel
implementation and duplicate work.

**Do this before Phase 1's rule work and before any registry building:**

1. **Fetch and inspect all remotes/branches.** `git fetch --all --prune`, then compare
   `origin/main`, `release-main`, local `main`, and any `feature/*` branches. Find where
   `scripts/core/novel_registry.py`, `test_novel_registry.py`, the GUI novel dropdown, and
   the `run_batch(novel_name=...)` dispatch actually live. The GitHub `main` README claims
   v0.9.0 — locate the commit/branch that matches it.
2. **Report the branch topology to the user** (which branch has what version, what's ahead/
   behind, whether histories diverged/were squashed) and **confirm the correct base** before
   proceeding. The Phase 0 note that `release-main @ 7a44b73` is "the verified current
   mainline" was based on the local clone only and is now in doubt — the GitHub `main`
   README describing v0.9.0 is evidence the real mainline is further ahead.
3. **Preferred path — reuse, don't rebuild:** if the v0.9.0 registry exists on a remote
   branch, re-create `feature/junk-strip-hardening` from **that** (the true current `main`),
   carrying over the Phase 0 baseline notes/scratch. Then the "dual-mode is already built"
   premise the rest of this plan assumes becomes *true*, and Phase 5 reverts to its original
   "confirm/prove dual-mode" scope (not "build it").
4. **Fallback path — build only if genuinely unrecoverable:** only if the registry cannot be
   found on any remote (truly lost, not just local-clone staleness) do you build it fresh:
   `scripts/core/novel_registry.py` (roster from `files/novel-index/*.txt` + `NovelDispatch`
   + `resolve_dispatch`), a universal-only fallback pipeline (its **own** neutral pipeline,
   not the LOTM stub — see the `novel_registry` caveat in Phase 5b), explicit Shadow Slave
   registration, `run_batch(novel_name=...)` dispatch with a universal-only default, the GUI
   dropdown, and `test_novel_registry.py`. Prove Shadow Slave output is byte-for-byte
   unchanged vs. the pre-registry v0.8.0 path (synthetic-text equivalence test).
5. **Record the outcome in `DECISIONS.md`** (created in Phase 10): which base branch was
   used, whether the registry was reused or rebuilt, and why. This is exactly the kind of
   non-obvious, easily-re-litigated decision the ADR log exists for.
6. **Checkpoint:** the branch base is confirmed with the user, the registry's real location
   is known, and the working branch contains a genuine dual-mode dispatch (reused or, only
   if forced, freshly built and proven SS-equivalent). **Do not proceed to Phase 2 until the
   base-branch question is settled** — everything downstream (especially Phases 5/5b) depends
   on it.
...
1. **Discover the available corpora first (don't assume paths).** Enumerate PDF-bearing
   folders under `files/test-files/`, `test-files/`, and `files/pdf-example-chapters/`.
   Report exactly what exists: which novel(s), how many PDFs each, rough sizes, chapter
   range from filenames/content, and whether each is tracked/ignored. Map each real folder
   to the role it plays here (dirty-Shadow-Slave evidence / clean-Shadow-Slave sample /
   Noble-Queen-or-other profile-less batch). If an expected dirtier batch is absent, say
   so and proceed with what's present per the "Locate the corpora" note in §2 above.
2. **Also inventory `files/study-examples/`** (used heavily in Phase 5b): list each
   `*-Word_List_Index/*_master_index.txt` and `_legacy/*` term file per novel (Noble Queen,
   Renegade Immortal, Reverend Insanity, Shadow Slave, Supreme Magus), and skim
   `sm_pdf_editor-v8.2.py` / `ss_pdf_editor-v1.py` / `scrape_noble_queen*.py` /
   `Shadow_Slave_Instructions-1.txt` for any **novel-specific edit rules or forced
   substitutions** encoded there (these feed Phase 5b). Don't implement anything yet —
   just catalog what novel-specific data exists.
3. Confirm "The Noble Queen" (and each other study-examples novel) currently resolves to
   the universal-only fallback (check `edit_details.resolve_novel_md_path` /
   `novel_registry`) so you know the true starting state before Phase 5b changes it.
4. Run raw `pdfplumber` extraction (`x_tolerance=5, y_tolerance=3`, matching
   `scripts/pdf/extractor.py`) against every PDF in each discovered corpus, into your
   Phase 0 scratch location — never into the corpus folders themselves, and never committed.
5. Scan the raw extracted text of **each corpus separately** (don't merge findings — note
...
6. **Do not auto-classify repeated text as junk.** For every candidate pattern:
...
7. Record findings **per corpus, per PDF where relevant**: pattern text (short, redacted
   ...
   reading the code).
8. **Deterministic sampling for any rendered/manual QA** (this phase or later ones) — pick
   ...
   Machine-detection (regex/text scanning) still covers **every** file in each corpus —
   sampling is only for the human/visual-inspection parts of Phase 3/Phase 4.
9. **Checkpoint:** report findings before proceeding to Phase 2. If patterns are
...
4. **Two distinct test layers** (this replaces any earlier version of this plan that
   ...
   - **Optional local-corpus integration tests**: clearly named/marked as corpus-backed
     (e.g. a `@pytest.mark.local_corpus` marker or a name prefix), operating on **whatever
     corpora Phase 1 actually discovered** (resolve their real paths at test time rather
     than hardcoding a folder name that may not exist) **only when present**, with an
     explicit, visible skip reason when absent — never a silent skip that could be mistaken
     for a pass. Add a way to make these mandatory on demand (a `--require-local-corpora`
     flag or equivalent) that **fails loudly** if invoked with a required corpus missing,
     so a real local QA run can't silently report green because the corpus wasn't there.
     Report processed/skipped/failed/sampled counts. Write all generated output to a temp
     directory, never into the corpus folder. Keep these fast enough (or bounded/sampled
     per Phase 1 §8) that they don't make ordinary
...
5. **Correctness, safety, and performance requirements for every new/changed pattern**
   ...
   - **Regex safety:** no unbounded/catastrophic-backtracking patterns (test with a very
     ...
     triggered by a generic word alone without corroborating context (per Phase 1 §6).
...
6. **Commit the 10 pinned Shadow Slave fixtures (user-approved fix).** These were never
   actually committed (`test-files/` is gitignored), so fixture-backed tests silently skip
   in a clean clone. Un-ignore just these 10 small PDFs (narrow the ignore rule or
   `git add -f` them) and commit them so the corpus-backed tests actually run everywhere.
   This is the **one** approved exception to "no corpus PDFs in git" — it's 10 small files,
   not a bulk corpus. (They still move to `files/test-files/` in Phase 8; keep them tracked
   through the move.)
7. **Checkpoint:** run the required committed test suite (`pytest`, not necessarily the
...
1. Run the full pipeline (post-Phase-2 fixes) against a **deterministically sampled**
   spread of the discovered corpora (per Phase 1 §8's sampling rule), mirroring the existing v0.5.0
...
1. Re-run the established programmatic garbage sweep (no spaced em/en/bar dashes, no
   ...
   against sampled output from the discovered corpora (per Phase 1 §8's sampling rule for anything
...
that basic/universal mode and Shadow-Slave-specific mode behave exactly as intended. This
confirms the registry established in Phase 0.5 (whether reused from the recovered v0.9.0
line or built fresh) works correctly against the real corpora before Phase 5b layers new
profiles on top.

1. Run the **Shadow Slave corpus discovered in Phase 1** (a dirty batch if present, else a
   sample of `webscraped_shadow_slave/`) through `run_batch` with
...
2. Run a **profile-less corpus discovered in Phase 1** (the Noble Queen batch if present;
   otherwise a Shadow Slave selection dispatched under a profile-less name) through
   `run_batch` in **basic/universal mode**: confirm via `novel_registry.resolve_dispatch`
   ...
   **no** Shadow-Slave-specific typo fixes. **Note:** if Phase 5b (below) is executed
   first and gives Noble Queen a real profile, use a *still-unregistered* novel for this
   universal-only proof instead — the point is to exercise the fallback, so it must be run
   against a novel that genuinely has no profile at that moment.
3. **Prove dispatch and rule provenance directly, with committed tests** (corpus-backed
   ...
   - Shadow Slave resolves to its registered profile; a profile-less novel resolves to the
   ...
   - A deliberately constructed **"bait" string matching an `SS_SPECIAL_FIXES` entry**
     ...
     untouched in a profile-less-mode run — this directly proves provenance rather than
     inferring it from absence of evidence.
...
4. **Checkpoint:** this confirms the architecture already described in
   ...
   dispatch-level proof and the real corpora actually available.

---

## Phase 5b — Author Real Per-Novel Profiles from `files/study-examples/`

**Goal:** turn the empty per-novel placeholders into real, working profiles using the
user's existing hand-built novel-specific data in `files/study-examples/`, proving the
"select a novel → get its unique edits" feature with more than just Shadow Slave. This is
a **data/porting exercise** on the already-validated multi-novel seam — **no core, rules,
or pdf module changes**.

**Scope (resolved with the user): author Noble Queen + Supreme Magus.** These are the two
novels with real portable data. Leave **Renegade Immortal** and **Reverend Insanity** as
universal-fallback placeholders — their study-examples master indexes are comment-only
headers with zero real terms (nothing to port). Also leave `circle-of-inevitability`,
`lord-of-the-mysteries`, and `re-monster` as placeholders (no study-examples data).

Per-novel notes:
- **Noble Queen** — 26 real terms; its `_legacy/noble_queen_do_not_touch.txt` is
  byte-identical to the master index (so no extra protected names to reconcile). The
  `scrape_noble_queen*.py` scrapers document source quirks including the webnovel.com
  decorative-Unicode homoglyph watermark (that's a *universal* junk-strip pattern handled in
  Phase 2, not a Noble-Queen-specific edit — don't duplicate it into the profile).
- **Supreme Magus** — 594-term master index + 4 legacy lists (the richest data).
  **Exclusion:** its old editor carries a **profanity-uncensor map** — this is
  spec-excluded (the pipeline is mechanical cleanup, not content alteration). **Do NOT port
  the profanity map** into `special_fixes.py` or anywhere else; port only genuine
  typo/name/formatting fixes.

For each of the two novels being onboarded:
1. **Protected-term index.** Populate `files/novel-index/<novel>.txt` from that novel's
   `study-examples/<Novel>-Word_List_Index/*_master_index.txt` (and relevant `_legacy/*`
   term lists). De-duplicate, one term per line, preserving any "do not touch" protected
   names. Keep the existing `novel-index` file format/conventions exactly.
2. **Canonical names + special fixes.** Fill in
   `scripts/profiles/<novel>/canonical_names.py` (the built-in canonical frozenset) and
   `scripts/profiles/<novel>/special_fixes.py` (forced typo substitutions), porting real
   entries found in the study-examples editor scripts / instruction files — **only
   genuine, evidence-backed novel-specific edits**, not invented ones, and **not** the
   Supreme Magus profanity map. Mirror the shape of the existing `profiles/shadow_slave/`
   files exactly.
3. **Pipeline + registry.** If the novel needs its own `pipelines/<novel>.py`, model it on
   `pipelines/shadow_slave.py` (same universal-rule order, its own profile data); register
   it in `novel_registry.py` so the dropdown dispatches to it. **Heed the standing
   `novel_registry` caveat:** if the universal-only fallback reuses the `lord_of_mysteries`
   stub *because its profile is empty*, and a real LOTM profile is ever authored, give the
   universal fallback its **own** dedicated neutral pipeline and register LOTM explicitly, so
   other profile-less novels don't inherit LOTM's fixes. (Phase 0.5's fallback build should
   already have given the universal path its own pipeline — confirm that's the case.)
4. **Edit-details doc.** Add `files/Novel-Edits-Details/<Novel-Name>.md` (mirroring
   `Shadow-Slave.md`) documenting that novel's specific edits in plain language.
5. **Tests.** Mirror `test_protection.py` / `test_edit_details.py` / `test_novel_registry.py`
   for the new novel: its protected terms survive the pipeline; its special fixes apply in
   its mode and **not** in others; it dispatches correctly; and — importantly — **adding it
   does not change Shadow Slave's output** (re-assert the SS equivalence guarantee). Use the
   discovered Noble Queen corpus if present; otherwise synthetic text built from its real
   index terms.
6. **Checkpoint:** targeted `pytest` green; the new novel is selectable and applies its own
   edits; Shadow Slave output is unchanged; universal-only fallback still works for novels
   left unauthored. Record which novels were onboarded and which were intentionally left as
   placeholders in the Phase 5b handoff entry. Because choosing a library/data source or
   leaving novels deferred is a non-obvious decision, add a `DECISIONS.md` entry (see Phase
   10 / the new DECISIONS doc) noting the scope choice.
...
2. **Audit the current repo against the spec and report every discrepancy in a Phase 8
   ...
   - Root contents vs. the spec's list. **`REPO-STRUCTURE.md` shows `.claude/` and
     `.codex/` already present** (with skills and `settings.local.json`), despite older
     notes claiming they were deleted — so **verify their current state and leave them
     as-is** unless something is actually missing; do not "restore" or overwrite existing
     agent config or skills. Only add what's genuinely absent (e.g. if a `CLAUDE.md`/
     `CODEX.md` pointer is missing, add a minimal one pointing back to `AI-WORKSPACE.md`).
     Validate any JSON you touch; never invent unsupported settings keys.
   - **`md-instructions/` doc set:** the repo has `BRIEFING.md`, `build-spec.md`,
     `CHANGELOG.md`, `decisions-template.md`, `EDITING-RULES.md`, `handoff.md`. Per the
     updated `AI-WORKSPACE.md`, the four *permanent* docs are Briefing, CHANGELOG,
     **DECISIONS**, and handoff. A `DECISIONS.md` does **not** yet exist (only the
     template) — create it in Phase 10 from `decisions-template.md`, seeding it with the
     non-obvious decisions this plan makes (Phase 5b scope, any `files/`→resources move,
     launcher non-alignment). Keep the existing lowercase `handoff.md` / `build-spec.md`
     casing; don't rename.
   ...
   - **`.test-tmp/` at the repo root** (visible in `REPO-STRUCTURE.md`) is stray test
     output that shouldn't be tracked — confirm it's gitignored (add it to `.gitignore` if
     not) and not staged; don't commit it. Do not delete the user's copy without asking,
     just make sure git ignores it.
   - **`.gitattributes`:** check whether the editor already has one. If it does, **add only
     ...
     overwrite it. After checkout, verify `.bat`/`.cmd` are actually CRLF and `.command`
     is actually LF.
   - **Launcher templates (temporary study copies — NOT keepers):** the root currently holds
     `Setup_and_Run-template.bat` / `Setup_and_Run-template.command` alongside the live
     `Setup_and_Run.bat` / `.command`. The user added the templates only as a reference for
     Claude Code to follow; **Phase 9 builds the single correct launcher per OS from them,
     then deletes the `-template` files.** The shipped repo must end with exactly one
     `Setup_and_Run.bat` and one `Setup_and_Run.command` (no `-template` variants). Do not
     treat the templates as permanent — they're removed by end of Phase 9.
   - **Do not delete other standard supporting files/folders just because the simplified
     `AI-WORKSPACE.md` root list doesn't name them.** `.gitattributes`, `LICENSE`,
     `.github/`, `pyproject.toml`, `pytest.ini`, or packaging/release config are legitimate
     exceptions if present — report them as intentional keepers, don't remove them to
     satisfy an overly literal reading of the
     root whitelist.
...
5. **`files/` runtime-classification audit (resolves a real contradiction — do this
   ...
   - **Development-only** (belongs under `files/`, not shipped): the pytest suite, test
     inputs, QA logs, study/reference material, other non-shipped fixtures. **Note the
     current repo has `test-files/shadow_slave/` at the REPO ROOT, not under `files/`** —
     per `AI-WORKSPACE.md` this dev-only test input belongs at `files/test-files/`, so move
     it there (`git mv test-files files/test-files`) as part of this reorg, and update any
     fixture-path references (e.g. in `test_batch.py`/`test_protection.py`/`conftest.py`).
     `files/study-examples/`, `files/pdf-example-chapters/`, and any `files/test-logs/` are
     also dev-only and stay under `files/`. Do **not** leave a genuine runtime dependency
     under `files/` just to shrink the reorg — that's exactly the contradiction being fixed.
...
8. **Commit discipline:** functional changes (Phases 1–7, including 5b) committed and verified *first*.
...
9. **Verify from multiple angles**, not just "tests pass from the repo root":
   ...
   - Use **semantic** before/after equivalence (extracted text, page count, structure —
     ...
     byte equality.
10. **Checkpoint:** report the Phase 8 audit findings (this is "Phase 8," not an earlier
    draft's "Phase 2" cross-reference) and how each was resolved — including `.claude`/
    `.codex` state, the `test-files/`→`files/test-files/` move, any runtime-resource
    relocation decision, and the `DECISIONS.md` creation — in the Phase 8 handoff entry.

---

## Phase 9 — Build the Single Launcher per OS (from the templates, then delete them)

**Goal:** produce **one** correct `Setup_and_Run.bat` and **one** `Setup_and_Run.command`
that meet the updated `AI-WORKSPACE.md` standard (numbered steps, self-healing venv,
idempotent install, correct scope prompts), built by following this repo's root
`Setup_and_Run-template.bat` / `Setup_and_Run-template.command` as the study pattern — and
then **delete the `-template` files** so the shipped repo has exactly one launcher per OS.
Do this without silently weakening the editor's own already-deliberate behavior (see
step 3).

**Reference (in priority order):** (1) this repo's root `Setup_and_Run-template.bat` /
`.command` — the study pattern to follow (per the updated `AI-WORKSPACE.md`); (2) the
current live `Setup_and_Run.bat` / `.command` in the root (what actually ships today, so
existing deliberate behavior isn't lost); (3) the `web-novel-scraper` launcher at
the commit SHA recorded in the Phase 0 baseline notes, as a proven real-world example of the
same pattern. Read all three before changing anything, and prefer the template's structure
where they differ.
...
12. **Delete the study templates.** Once the single `Setup_and_Run.bat` and
    `Setup_and_Run.command` are built and passing, **`git rm` the
    `Setup_and_Run-template.bat` and `Setup_and_Run-template.command` files** — they were
    reference-only. Confirm the repo root now contains exactly one launcher per OS and no
    `-template` variant remains (check the root listing and `.gitignore` doesn't
    accidentally leave an untracked copy behind). Note this deletion in the Phase 9 handoff
    entry.
13. **Checkpoint:** launcher syntax checks + targeted `pytest` green; exactly one launcher
    per OS in the root, templates removed. Note in the Phase 9 handoff entry exactly which
    template/scraper patterns were adopted structurally and which editor-specific behaviors
    (step 3, and the consent/health-check hardening) were kept or strengthened, and why.
...
1. **`md-instructions/EDITING-RULES.md`** — update the Stage 1.5 "Evidence base" section
   with the real patterns found in Phase 1 from the discovered corpora (keep old evidence, add new,
...
4. **`md-instructions/CHANGELOG.md`** — replace the Phase 0 placeholder (if one was added
   ...
   sweep result, Phase 5b new per-novel profiles (list which novels were onboarded), Phase 6
   PDF-build alignment (**state plainly** whether the orphan-page
...
5. **`md-instructions/BRIEFING.md`** — bump version/phase, update "What Was Just Built,"
   fold the corpora actually used into "What Is Working" (as local QA evidence), cover the
   new per-novel profiles from Phase 5b, update the orphan-page Deferred Feature note per
   what Phase 6 actually did, reflect any `files/`→`resources/` runtime-resource move from
   Phase 8, and update every path reference (e.g. the `batch_runner` path-resolution note)
   to match `scripts/Universal/` and the `test-files/`→`files/test-files/` move.
6. **`md-instructions/DECISIONS.md`** (new — create from `decisions-template.md` per the
   updated `AI-WORKSPACE.md`, which now lists DECISIONS as the fourth permanent doc). Seed
   it, newest-on-top, with the non-obvious decisions this plan made: the Phase 5b scope
   choice (which novels onboarded vs. deferred, and why); the runtime-resource location
   decision from Phase 8 (whether novel data moved out of `files/` and where, or stayed per
   build-spec after user confirmation); the deliberate launcher Python-version-gate
   non-alignment (editor blocks, scraper warns); and any orphan-page-handling decision
   (prevention vs. deferred deletion). Append-only, signed and dated, per the template.
7. **`md-instructions/handoff.md`** — append a dated work-log entry, plus a Session Sync
...
2. **Phase 0.5** reconciled branches, confirmed the correct base with the user, and made
   the working branch contain a real dual-mode registry (reused from the recovered v0.9.0
   line, or — only if unrecoverable — freshly built and proven SS-equivalent).
3. **Phases 1–7 (including 5b)** (functional work) each ran their own targeted `pytest` subset — and the
...
4. **Commit the verified functional work** (Phases 1–7, including 5b).
5. **Phase 8** performs the reorganization.
6. Run the **full `pytest` suite from the new structure**, plus the path-smoke and
   release-ZIP-without-`files/` checks from Phase 8 §9.
7. **Phase 9** hardens and tests the launchers against the **final** paths.
8. **Phase 10** finalizes README, BRIEFING, CHANGELOG, EDITING-RULES, handoff, DECISIONS, and any
   other permanent docs.
9. **Compare final corpus PDF SHA-256 hashes against the Phase 0 hashes** to prove the
   source files were never touched.
10. **Delete this instruction file**
    (`md-instructions/Instructions_Phase10_JunkStrip_And_QA.md`).
11. Run the complete official `python scripts/verify.py` against the **actual final tree,
    with the instruction file already deleted**. Record pass/fail/skip/duration.
12. Record that result in `handoff.md`.
13. **Because recording that result changed `handoff.md` after verification, run the
...
14. Inspect `git status --short` and the staged file list; **confirm no corpus PDF
    (except the 10 approved Shadow Slave fixtures), extracted chapter text, QA scratch,
    temporary/`.part` PDF, or local `.venv` artifact is staged**.
15. Commit the final documentation/verification state.
16. **Do not merge to `main`.** Leave the feature branch unmerged for the user's review.
...
- [ ] The repo root contains exactly **one** `Setup_and_Run.bat` and **one**
      `Setup_and_Run.command`; the `-template` study copies were deleted (Phase 9).
...
- [ ] Baseline (Phase 0) and final `pytest` counts are reconciled — every new skip has a
      stated reason.
- [ ] The working branch contains a real dual-mode dispatch registry — reused from the
      recovered v0.9.0 line if Phase 0.5 found it, or freshly built and proven byte-for-byte
      SS-equivalent only if it was genuinely unrecoverable. The base-branch choice was
      confirmed with the user and recorded in `DECISIONS.md`.
- [ ] **Both** Noble Queen and Supreme Magus have real profiles authored from
      `files/study-examples/`, are selectable in the GUI, apply their own edits, and do
      **not** change Shadow Slave's output. Renegade Immortal / Reverend Insanity (and other
      dataless novels) remain clean universal-only fallbacks.
- [ ] The Supreme Magus **profanity-uncensor map was NOT ported** into any profile or rule.
- [ ] The 10 pinned Shadow Slave fixtures are now actually committed (un-ignored), so
      fixture-backed tests no longer silently skip in a clean clone.
- [ ] `DECISIONS.md` exists (created from the template) and records this round's non-obvious
      choices (branch/registry reconciliation, Phase 5b scope, runtime-resource location,
      launcher non-alignment).
- [ ] `test-files/` was moved to `files/test-files/` and all fixture-path references updated.
...
- **Stay in scope.** Junk-strip hardening + editorial QA + TTS verification + dual-mode
  proof + **per-novel profile authoring from study-examples (data/porting only)** +
  PDF-build alignment + GUI consistency + repo reorg + launcher hardening. **Not** a
  rewrite of the pipeline architecture's *behavior*, and no new `core`/`rules`/`pdf`
  modules — Phase 5b adds *data and profile files* on the existing validated seam, not new
  engine code.
