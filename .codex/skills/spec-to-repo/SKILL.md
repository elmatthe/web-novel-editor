...
Turn a natural-language project specification into a complete, runnable starter repository. Not a template filler â€” a spec interpreter that generates real, working code for any stack.
...
**Not this skill** when the user wants a SaaS app with Stripe + Auth specifically â€” use `product-team/saas-scaffolder` instead.

## Core Workflow

### Phase 1 â€” Parse & Interpret
...
**Database:** [yes/no â€” engine]
**Auth:** [yes/no â€” method]
**Deploy:** [target]
...
1. **Select template** â€” Match to a stack template from `references/stack-templates.md`
2. **Define file tree** â€” List every file that will be created
3. **Map features to files** â€” Each feature gets at minimum one file/component
4. **Design database schema** â€” If applicable, define tables/collections with fields and types
5. **Identify dependencies** â€” List every package with version constraints
6. **Plan API routes** â€” If applicable, list every endpoint with method, path, request/response shape
...
task-api/
â”œâ”€â”€ README.md
â”œâ”€â”€ .env.example              # API_KEY, DATABASE_URL
â”œâ”€â”€ .gitignore
â”œâ”€â”€ .github/workflows/ci.yml
â”œâ”€â”€ requirements.txt          # fastapi, uvicorn, sqlalchemy, pytest
â”œâ”€â”€ main.py                   # FastAPI app, CORS, lifespan
â”œâ”€â”€ models.py                 # SQLAlchemy Task model
â”œâ”€â”€ schemas.py                # Pydantic request/response schemas
â”œâ”€â”€ database.py               # SQLite engine + session
â”œâ”€â”€ auth.py                   # API key middleware
â”œâ”€â”€ routers/
â”‚   â””â”€â”€ tasks.py              # CRUD endpoints
â””â”€â”€ tests/
    â””â”€â”€ test_tasks.py         # Smoke tests for each endpoint
```
...
recipe-share/
â”œâ”€â”€ README.md
â”œâ”€â”€ .env.example              # DATABASE_URL, NEXTAUTH_SECRET, NEXTAUTH_URL
â”œâ”€â”€ .gitignore
â”œâ”€â”€ .github/workflows/ci.yml
â”œâ”€â”€ package.json              # next, react, tailwindcss, prisma, next-auth
â”œâ”€â”€ tailwind.config.ts
â”œâ”€â”€ tsconfig.json
â”œâ”€â”€ next.config.ts
â”œâ”€â”€ prisma/
â”‚   â””â”€â”€ schema.prisma         # User, Recipe, Ingredient, Favorite models
â”œâ”€â”€ src/
â”‚   â”œâ”€â”€ app/
â”‚   â”‚   â”œâ”€â”€ layout.tsx
â”‚   â”‚   â”œâ”€â”€ page.tsx          # Homepage â€” recipe feed
â”‚   â”‚   â”œâ”€â”€ recipes/
â”‚   â”‚   â”‚   â”œâ”€â”€ page.tsx      # Browse recipes
â”‚   â”‚   â”‚   â”œâ”€â”€ [id]/page.tsx # Recipe detail
â”‚   â”‚   â”‚   â””â”€â”€ new/page.tsx  # Create recipe form
â”‚   â”‚   â””â”€â”€ api/
â”‚   â”‚       â”œâ”€â”€ auth/[...nextauth]/route.ts
â”‚   â”‚       â””â”€â”€ recipes/route.ts
â”‚   â”œâ”€â”€ components/
...
â”‚       â”œâ”€â”€ cli.py            # argparse commands
â”‚       â”œâ”€â”€ database.py       # SQLite operations
â”‚       â”œâ”€â”€ models.py         # Expense dataclass
â”‚       â””â”€â”€ formatters.py     # Table + CSV output
â””â”€â”€ tests/
    â””â”€â”€ test_cli.py
```
...
| Anti-pattern | Fix |
|---|---|
| **Placeholder code** â€” `// TODO: implement`, `pass`, empty function bodies | Every function has a real implementation. If complex, implement a working simplified version. |
| **Stack override** â€” picking Next.js when the user said Flask | Always honor explicit tech preferences. Only infer when the user doesn't specify. |
| **Missing .gitignore** â€” committing node_modules or .env | Generate stack-appropriate .gitignore as one of the first files. |
| **Phantom imports** â€” importing packages not in the manifest | Cross-check every import against package.json / requirements.txt before finishing. |
| **Over-engineering MVP** â€” adding Redis caching, rate limiting, WebSockets to a v1 | Build the minimum that works. The user can iterate. |
| **Ignoring stated preferences** â€” user says "PostgreSQL" and you generate MongoDB | Parse the spec carefully. Explicit preferences are non-negotiable. |
| **Missing env vars** â€” code reads `process.env.X` but `.env.example` doesn't list it | Every env var used in code must appear in `.env.example` with a comment. |
| **No tests** â€” shipping a repo with zero test files | At minimum: one smoke test per API endpoint or one test per core function. |
| **Hallucinated APIs** â€” generating code that calls library methods that don't exist | Stick to well-documented, stable APIs. When unsure, use the simplest approach. |
...
1. **MVP** â€” Core feature only, working end-to-end
2. **Auth** â€” Add authentication if requested
3. **Polish** â€” Error handling, validation, loading states
4. **Deploy** â€” Docker, CI, deploy config

Ask the user after MVP: "Core is working. Want me to add auth/polish/deploy next, or iterate on what's here?"

## Cross-References

- Related: `product-team/saas-scaffolder` â€” SaaS-specific scaffolding (Next.js + Stripe + Auth)
- Related: `engineering/spec-driven-workflow` â€” spec-first development methodology
- Related: `engineering/database-designer` â€” database schema design patterns
- Related: `engineering-team/senior-fullstack` â€” full-stack implementation patterns
