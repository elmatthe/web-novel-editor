...
### Test Generation â€” Input â†’ Output (Pytest)
...
### Coverage Analysis â€” Sample P0/P1/P2 Output
...
**Sample output:**
```
Coverage Report â€” Overall: 63% (threshold: 80%)

P0 â€” Critical gaps (uncovered error paths):
  auth/login.py:42-58   handle_expired_token()       0% covered
  payments/process.py:91-110  handle_payment_failure()   0% covered

P1 â€” High-value gaps (core logic branches):
  users/service.py:77   update_profile() â€” else branch  0% covered
  orders/cart.py:134    apply_discount() â€” zero-qty guard  0% covered

P2 â€” Low-risk gaps (utility / helper functions):
...
1. **Write or receive a spec** â€” stored in `specs/<feature>.md`
2. **Extract acceptance criteria** â€” each criterion becomes one or more test cases
3. **Write failing tests (RED)** â€” one test per acceptance criterion
4. **Implement minimal code (GREEN)** â€” satisfy each test in order
5. **Refactor** â€” clean up while all tests stay green
...
project/
â”œâ”€â”€ specs/
â”‚   â”œâ”€â”€ user-auth.md          # Feature spec with acceptance criteria
â”‚   â”œâ”€â”€ payment-processing.md
â”‚   â””â”€â”€ notification-system.md
â”œâ”€â”€ tests/
â”‚   â”œâ”€â”€ test_user_auth.py     # Tests derived from specs/user-auth.md
...
# tests/test_pricing.py â€” parametrize for multiple cases
import pytest
from app.pricing import calculate_discount

@pytest.mark.parametrize("subtotal, expected_discount", [
    (50.0, 0.0),       # Below threshold â€” no discount
    (100.0, 5.0),      # 5% tier
    (250.0, 25.0),     # 10% tier
    (500.0, 75.0),     # 15% tier
])
...
- **Ambiguous requirements** â€” the spec or user story has conflicting or unclear acceptance criteria
- **Missing edge cases** â€” you cannot determine boundary values without domain knowledge (e.g., max allowed transaction amount)
- **Test count exceeds 50** â€” large test suites need human review before committing; present a summary and ask which areas to prioritize
- **External dependencies unclear** â€” the feature relies on third-party APIs or services with undocumented behavior
- **Security-sensitive logic** â€” authentication, authorization, encryption, or payment flows require human sign-off on test scenarios

### Continue Autonomously When

- **Clear spec with numbered acceptance criteria** â€” each criterion maps directly to tests
- **Straightforward CRUD operations** â€” create, read, update, delete with well-defined models
- **Well-defined API contracts** â€” OpenAPI spec or typed interfaces available
- **Pure functions** â€” deterministic input/output with no side effects
- **Existing test patterns** â€” the codebase already has similar tests to follow
...
- **100% line coverage != good tests** â€” coverage tells you code was executed, not that it was verified
- **Catches weak assertions** â€” tests that run code but assert nothing meaningful
- **Finds missing boundary tests** â€” mutants that change `<` to `<=` expose off-by-one gaps
- **Quantifiable quality metric** â€” mutation score (% mutants killed) is a stronger signal than coverage %
...
| Skill | Relationship |
|-------|-------------|
| `engineering/spec-driven-workflow` | Spec â†’ acceptance criteria â†’ test extraction pipeline |
